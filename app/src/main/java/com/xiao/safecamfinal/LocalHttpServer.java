...
final class LocalHttpServer extends NanoHTTPD {
    ...
    private Response livePage() {
        ...
        String html = pageStart("SafeCam Final", p)
                + "<main class='wrap'>"
    ...
    private Response settingsPage(Map<String, String> params) {
        ...
        String html = pageStart("Remote Settings", p)
                ...
                + "</form>"
                + "<div class='note'><b>说明：</b>当前稳定版是图片循环记录，不是 24fps MP4 录像。24fps 会在下一版用 H.264/MP4 分段录像重做。</div>"
    ...
    private Response guidePage() {
        ...
        String html = pageStart("Remote View", p)
                ...
                + "<section class='card'><h2>Tailscale / ZeroTier</h2><ol><li>旧手机和查看手机都安装 Tailscale 或 ZeroTier。</li><li>加入同一个账号或虚拟网络。</li><li>旧手机 App 如果显示 100.x.x.x 地址，外出手机浏览器打开它。</li><li>示例：<code>http://100.x.x.x:8080/?pin=123456</code></li></ol><p>摄像头不够广角时，优先把手机后移或加外接广角夹镜。稳定版不再使用容易报错的实验广角接口。</p></section>"
                + "</main></body></html>";
        return newFixedLengthResponse(Response.Status.OK, "text/html; charset=utf-8", html);
    }
    ...
    private String pageStart(String title, String p) {
        return "<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'>"
                ...
                + "<header><div class='brand'>● SafeCam Final</div><nav>"
                + "<a href='/?pin=" + p + "'>实时</a>"
